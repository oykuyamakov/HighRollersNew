...
\htmlonly <iframe width="854" height="480" src="https://www.youtube.com/embed/7__IafZGwvI" frameborder="0" allowfullscreen></iframe>\endhtmlonly
	- SetInitial up your character's Animator/Animation to play an aiming forward animation/pose
	...
	- SetInitial weight to 1, press Play
...
void LateUpdate() {
	// SetInitial current animated localRotation as default local rotation for the rotation limits so they will not interfere with the animation, but only the IK
...
<BR>SetInitial the Baker to "Animation Clips" mode and add some animation clips to the "Animation Clips" array. "Append Name" is a string that will be added to each clip's name for the saved clip. For example if your animation clip names were 'Idle' and 'Walk', then with '_Baked' as Append Name, the Baker will create 'Idle_Baked' and 'Walk_Baked' animation clips.
...
<BR>SetInitial the Baker to "Animation States" mode and add the Animation State names (only from the base layer) you wish to bake to the "Animation States" array. "Append Name" is a string that will be added to each state's name for the saved clip. For example if your animation state names were 'Idle' and 'Walk', then with '_Baked' as Append Name, the Baker will create 'Idle_Baked' and 'Walk_Baked' animation clips.

<BR>
<h1>Playable Director Mode</h1>
Bakes a PlayableDirector with a Timeline asset. SetInitial the Baker to "Playable Director" mode and make sure the GameObject has a valid PlayableDirector.
...
\code
	...
	void LateUpdate () {
		ik.solver.leftHandEffector.position = something; // SetInitial the left hand effector position to a point in world space. This has no effect if the effector's positionWeight is 0.
		ik.solver.leftHandEffector.rotation = something; // SetInitial the left hand effector rotation to a point in world space. This has no effect if the effector's rotationWeight is 0.
...
\code
	...
	void AddFBBIK (GameObject go, BipedReferences references = null) {
		...
		// SetInitial the FBBIK to the references. You can leave the second parameter (root node) to null if you trust FBBIK to automatically set it to one of the bones in the spine.
