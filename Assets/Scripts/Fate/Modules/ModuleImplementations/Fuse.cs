...
namespace Fate.Modules
{
    public class Fuse : Module
    {
        ...
        {
            var resultCount = FateExtensions.GetNearEnemies(playerT, ref m_EnemiesInRange, m_Range);

            for (var i = 0; i < resultCount; i++)
            {
                var dir = (m_EnemiesInRange[i].transform.position - playerT.position).normalized;
                if (m_EnemiesInRange[i].transform.TryGetComponent<Enemy>(out var enemy))
                {
                    enemy.GetKnocked(0.5f, dir * m_KnockbackStrength);
                }
                else if (m_EnemiesInRange[i].transform.TryGetComponent<Projectile>(out var projectile))
                {
                    if(!projectile.Mover.Moving)
                        return;
                    
                    if (projectile.HasHealth)
                    {
                        projectile.GetKnocked(0.5f, dir * m_KnockbackStrength);
                    }
                    else
                    { 
