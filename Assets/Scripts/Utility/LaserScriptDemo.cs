...
namespace GAP_LaserSystem{
	...
	public class PSList2 {

		public enum PSLIST_TYPE2 {start, middle, end};
		public GameObject _parent;
		public List<ParticleSystem> _listPS = new List<ParticleSystem>();
		public PSLIST_TYPE2 _psType;
	...
	public class LaserScriptDemo : MonoBehaviour {
		...
		public void EnableLaser (Vector3 endpoint)
		{
		...
		public void UpdateLaser () {
			...
			else 
			{
				...
				if (startVFX != null && useStart)
					RotateToMouse (startVFX, EndPosition);

				if (middleVFX != null && useMiddle)
					RotateToMouse (middleVFX, EndPosition);		
				...
				if (trailVFX != null && useTrail) {
					if (trails [0]._emittingTrail == false) {
						trails [0]._trailPosition = EndPosition;
				...
				{	
					RaycastHit hit;
					var direction = EndPosition - firePoint.transform.position;
					Ray ray = new Ray (firePoint.transform.position, direction);
					if(Physics.Raycast (ray, out hit)){
						for(int i = 0; i<lineRenderers.Count; i++)
							RecursiveBounces (lineRenderers[i], EndPosition, Vector3.Reflect(ray.direction, hit.normal), bounces, maximumLength - Vector3.Distance(lineRenderers[i].GetPosition(0), lineRenderers[i].GetPosition(1)));
					}
				}
				else
				{
